...
import com.progress.progress.repositories.ConstructionCompanyRepository;
...
public class ConstructionCompanyService implements IConstructionCompanyService{
	/** 
     * 注入constructionCompanyRepository
     */
	@Autowired
	...
	@Override
	public QueryResultObject getConstructionCompanyById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<ConstructionCompany> optional = constructionCompanyRepository.findById(temp_id);	
		ConstructionCompany constructionCompany = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(constructionCompany);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			constructionCompanyRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public ConstructionCompany saveConstructionCompany(Map<String,Object> map) throws Exception{
		validateService.validateWithException(ConstructionCompany.class,map);
		ConstructionCompany constructionCompany = new ConstructionCompany();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<ConstructionCompany> optional = constructionCompanyRepository.findById(id);
			constructionCompany = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, constructionCompany,  "id");
		}else{
			CrudUtils.transMap2Bean(map, constructionCompany);
		}
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<ConstructionCompany> specification = new Specification<ConstructionCompany>() {
			@Override
			public Predicate toPredicate(Root<ConstructionCompany> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<ConstructionCompany> constructionCompany = constructionCompanyRepository.findAll(specification,request);
		List<ConstructionCompany> result = new ArrayList<ConstructionCompany>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = constructionCompany.getContent();
			count = constructionCompany.getTotalElements();
		}
