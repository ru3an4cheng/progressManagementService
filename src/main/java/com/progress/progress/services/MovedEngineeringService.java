...
import com.progress.progress.repositories.MovedEngineeringRepository;
 ...
 */
@Service
public class MovedEngineeringService implements IMovedEngineeringService{
	/** 
     * 注入movedEngineeringRepository
     */
	@Autowired
	private MovedEngineeringRepository movedEngineeringRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getMovedEngineeringById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<MovedEngineering> optional = movedEngineeringRepository.findById(temp_id);	
		MovedEngineering movedEngineering = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(movedEngineering);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			movedEngineeringRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public MovedEngineering saveMovedEngineering(Map<String,Object> map) throws Exception{
		validateService.validateWithException(MovedEngineering.class,map);
		MovedEngineering movedEngineering = new MovedEngineering();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<MovedEngineering> optional = movedEngineeringRepository.findById(id);
			movedEngineering = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, movedEngineering,  "id");
		}else{
			CrudUtils.transMap2Bean(map, movedEngineering);
		}
		return movedEngineeringRepository.save(movedEngineering);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<MovedEngineering> specification = new Specification<MovedEngineering>() {
			@Override
			public Predicate toPredicate(Root<MovedEngineering> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<MovedEngineering> movedEngineering = movedEngineeringRepository.findAll(specification,request);
		List<MovedEngineering> result = new ArrayList<MovedEngineering>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = movedEngineering.getContent();
			count = movedEngineering.getTotalElements();
		}
