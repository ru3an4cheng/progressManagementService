...
import com.progress.progress.repositories.ProjectNodesRepository;
 ...
 */
@Service
public class ProjectNodesService implements IProjectNodesService{
	/** 
     * 注入projectNodesRepository
     */
	@Autowired
	private ProjectNodesRepository projectNodesRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getProjectNodesById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<ProjectNodes> optional = projectNodesRepository.findById(temp_id);	
		ProjectNodes projectNodes = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(projectNodes);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			projectNodesRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public ProjectNodes saveProjectNodes(Map<String,Object> map) throws Exception{
		validateService.validateWithException(ProjectNodes.class,map);
		ProjectNodes projectNodes = new ProjectNodes();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<ProjectNodes> optional = projectNodesRepository.findById(id);
			projectNodes = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, projectNodes,  "id");
		}else{
			CrudUtils.transMap2Bean(map, projectNodes);
		}
		return projectNodesRepository.save(projectNodes);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<ProjectNodes> specification = new Specification<ProjectNodes>() {
			@Override
			public Predicate toPredicate(Root<ProjectNodes> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<ProjectNodes> projectNodes = projectNodesRepository.findAll(specification,request);
		List<ProjectNodes> result = new ArrayList<ProjectNodes>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = projectNodes.getContent();
			count = projectNodes.getTotalElements();
		}
