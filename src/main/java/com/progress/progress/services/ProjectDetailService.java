...
import com.progress.progress.repositories.ProjectDetailRepository;
...
public class ProjectDetailService implements IProjectDetailService{
	...
	public QueryResultObject getProjectDetailById(String id) {
		...
		Optional<ProjectDetail> optional = projectDetailRepository.findById(temp_id);	
		ProjectDetail projectDetail = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(projectDetail);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			projectDetailRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public ProjectDetail saveProjectDetail(Map<String,Object> map) throws Exception{
		validateService.validateWithException(ProjectDetail.class,map);
		ProjectDetail projectDetail = new ProjectDetail();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<ProjectDetail> optional = projectDetailRepository.findById(id);
			projectDetail = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, projectDetail,  "id");
		}else{
			CrudUtils.transMap2Bean(map, projectDetail);
		}
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<ProjectDetail> specification = new Specification<ProjectDetail>() {
			@Override
			public Predicate toPredicate(Root<ProjectDetail> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<ProjectDetail> projectDetail = projectDetailRepository.findAll(specification,request);
		List<ProjectDetail> result = new ArrayList<ProjectDetail>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = projectDetail.getContent();
			count = projectDetail.getTotalElements();
		}
