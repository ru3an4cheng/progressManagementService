...
import com.progress.progress.repositories.MaterialArrivalManagementRepository;
 ...
 */
@Service
public class MaterialArrivalManagementService implements IMaterialArrivalManagementService{
	/** 
     * 注入materialArrivalManagementRepository
     */
	@Autowired
	private MaterialArrivalManagementRepository materialArrivalManagementRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getMaterialArrivalManagementById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<MaterialArrivalManagement> optional = materialArrivalManagementRepository.findById(temp_id);	
		MaterialArrivalManagement materialArrivalManagement = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(materialArrivalManagement);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			materialArrivalManagementRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public MaterialArrivalManagement saveMaterialArrivalManagement(Map<String,Object> map) throws Exception{
		validateService.validateWithException(MaterialArrivalManagement.class,map);
		MaterialArrivalManagement materialArrivalManagement = new MaterialArrivalManagement();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<MaterialArrivalManagement> optional = materialArrivalManagementRepository.findById(id);
			materialArrivalManagement = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, materialArrivalManagement,  "id");
		}else{
			CrudUtils.transMap2Bean(map, materialArrivalManagement);
		}
		return materialArrivalManagementRepository.save(materialArrivalManagement);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<MaterialArrivalManagement> specification = new Specification<MaterialArrivalManagement>() {
			@Override
			public Predicate toPredicate(Root<MaterialArrivalManagement> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<MaterialArrivalManagement> materialArrivalManagement = materialArrivalManagementRepository.findAll(specification,request);
		List<MaterialArrivalManagement> result = new ArrayList<MaterialArrivalManagement>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = materialArrivalManagement.getContent();
			count = materialArrivalManagement.getTotalElements();
		}
