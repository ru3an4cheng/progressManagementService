...
import com.progress.progress.repositories.ProjectSubitemRepository;
 ...
 */
@Service
public class ProjectSubitemService implements IProjectSubitemService{
	/** 
     * 注入projectSubitemRepository
     */
	@Autowired
	private ProjectSubitemRepository projectSubitemRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getProjectSubitemById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<ProjectSubitem> optional = projectSubitemRepository.findById(temp_id);	
		ProjectSubitem projectSubitem = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(projectSubitem);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			projectSubitemRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public ProjectSubitem saveProjectSubitem(Map<String,Object> map) throws Exception{
		validateService.validateWithException(ProjectSubitem.class,map);
		ProjectSubitem projectSubitem = new ProjectSubitem();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<ProjectSubitem> optional = projectSubitemRepository.findById(id);
			projectSubitem = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, projectSubitem,  "id");
		}else{
			CrudUtils.transMap2Bean(map, projectSubitem);
		}
		return projectSubitemRepository.save(projectSubitem);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<ProjectSubitem> specification = new Specification<ProjectSubitem>() {
			@Override
			public Predicate toPredicate(Root<ProjectSubitem> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<ProjectSubitem> projectSubitem = projectSubitemRepository.findAll(specification,request);
		List<ProjectSubitem> result = new ArrayList<ProjectSubitem>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = projectSubitem.getContent();
			count = projectSubitem.getTotalElements();
		}
