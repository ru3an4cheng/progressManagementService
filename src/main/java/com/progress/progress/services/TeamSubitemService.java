...
import com.progress.progress.repositories.TeamSubitemRepository;
 ...
 */
@Service
public class TeamSubitemService implements ITeamSubitemService{
	/** 
     * 注入teamSubitemRepository
     */
	@Autowired
	private TeamSubitemRepository teamSubitemRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getTeamSubitemById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<TeamSubitem> optional = teamSubitemRepository.findById(temp_id);	
		TeamSubitem teamSubitem = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(teamSubitem);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			teamSubitemRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public TeamSubitem saveTeamSubitem(Map<String,Object> map) throws Exception{
		validateService.validateWithException(TeamSubitem.class,map);
		TeamSubitem teamSubitem = new TeamSubitem();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<TeamSubitem> optional = teamSubitemRepository.findById(id);
			teamSubitem = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, teamSubitem,  "id");
		}else{
			CrudUtils.transMap2Bean(map, teamSubitem);
		}
		return teamSubitemRepository.save(teamSubitem);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<TeamSubitem> specification = new Specification<TeamSubitem>() {
			@Override
			public Predicate toPredicate(Root<TeamSubitem> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<TeamSubitem> teamSubitem = teamSubitemRepository.findAll(specification,request);
		List<TeamSubitem> result = new ArrayList<TeamSubitem>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = teamSubitem.getContent();
			count = teamSubitem.getTotalElements();
		}
