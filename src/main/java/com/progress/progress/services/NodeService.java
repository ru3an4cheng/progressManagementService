...
import com.progress.progress.repositories.NodeRepository;
...
public class NodeService implements INodeService{
	...
	@Override
	public QueryResultObject getNodeById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<Node> optional = nodeRepository.findById(temp_id);	
		Node node = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(node);
	}
	@Override
	...
	@Override
	public Node saveNode(Map<String,Object> map) throws Exception{
		validateService.validateWithException(Node.class,map);
		Node node = new Node();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<Node> optional = nodeRepository.findById(id);
			node = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, node,  "id");
		}else{
			CrudUtils.transMap2Bean(map, node);
		}
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Page<Node> node = nodeRepository.findAll(specification,request);
		List<Node> result = new ArrayList<Node>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = node.getContent();
			count = node.getTotalElements();
		}
