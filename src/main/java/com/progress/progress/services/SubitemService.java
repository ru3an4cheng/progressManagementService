...
import com.progress.progress.repositories.SubitemRepository;
 ...
 */
@Service
public class SubitemService implements ISubitemService{
	/** 
     * 注入subitemRepository
     */
	@Autowired
	...
	@Override
	public QueryResultObject getSubitemById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<Subitem> optional = subitemRepository.findById(temp_id);	
		Subitem subitem = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(subitem);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			subitemRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public Subitem saveSubitem(Map<String,Object> map) throws Exception{
		validateService.validateWithException(Subitem.class,map);
		Subitem subitem = new Subitem();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<Subitem> optional = subitemRepository.findById(id);
			subitem = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, subitem,  "id");
		}else{
			CrudUtils.transMap2Bean(map, subitem);
		}
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<Subitem> specification = new Specification<Subitem>() {
			@Override
			public Predicate toPredicate(Root<Subitem> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<Subitem> subitem = subitemRepository.findAll(specification,request);
		List<Subitem> result = new ArrayList<Subitem>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = subitem.getContent();
			count = subitem.getTotalElements();
		}
