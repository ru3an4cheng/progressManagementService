...
import com.progress.progress.repositories.ConstructionTeamRepository;
 ...
 */
@Service
public class ConstructionTeamService implements IConstructionTeamService{
	/** 
     * 注入constructionTeamRepository
     */
	@Autowired
	...
	@Override
	public QueryResultObject getConstructionTeamById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<ConstructionTeam> optional = constructionTeamRepository.findById(temp_id);	
		ConstructionTeam constructionTeam = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(constructionTeam);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			constructionTeamRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public ConstructionTeam saveConstructionTeam(Map<String,Object> map) throws Exception{
		validateService.validateWithException(ConstructionTeam.class,map);
		ConstructionTeam constructionTeam = new ConstructionTeam();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<ConstructionTeam> optional = constructionTeamRepository.findById(id);
			constructionTeam = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, constructionTeam,  "id");
		}else{
			CrudUtils.transMap2Bean(map, constructionTeam);
		}
		return constructionTeamRepository.save(constructionTeam);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<ConstructionTeam> specification = new Specification<ConstructionTeam>() {
			@Override
			public Predicate toPredicate(Root<ConstructionTeam> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<ConstructionTeam> constructionTeam = constructionTeamRepository.findAll(specification,request);
		List<ConstructionTeam> result = new ArrayList<ConstructionTeam>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = constructionTeam.getContent();
			count = constructionTeam.getTotalElements();
		}
