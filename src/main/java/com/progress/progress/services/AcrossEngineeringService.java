...
import com.progress.progress.repositories.AcrossEngineeringRepository;
 ...
 */
@Service
public class AcrossEngineeringService implements IAcrossEngineeringService{
	/** 
     * 注入acrossEngineeringRepository
     */
	@Autowired
	private AcrossEngineeringRepository acrossEngineeringRepository;
	@Autowired
	private ValidateService validateService;
	
	@Override
	public QueryResultObject getAcrossEngineeringById(String id) {
		Long temp_id = null;
		if(!StringUtil.isNullOrEmpty(id) && !"null".equals(id)){
		  temp_id = Long.parseLong(id);
		}
		Optional<AcrossEngineering> optional = acrossEngineeringRepository.findById(temp_id);	
		AcrossEngineering acrossEngineering = optional.isPresent()?optional.get():null;
		return RestUtils.wrappQueryResult(acrossEngineering);
	}
	@Override
	public void remove(IDRequestObject idObject) {
		...
		for (String id : ids){
			acrossEngineeringRepository.deleteById(Long.valueOf(id));
		}
	}
	@Override
	public AcrossEngineering saveAcrossEngineering(Map<String,Object> map) throws Exception{
		validateService.validateWithException(AcrossEngineering.class,map);
		AcrossEngineering acrossEngineering = new AcrossEngineering();
		if (map.containsKey("id")) {
		Long id = Long.parseLong(map.get("id").toString());
			
			Optional<AcrossEngineering> optional = acrossEngineeringRepository.findById(id);
			acrossEngineering = optional.isPresent()?optional.get():null;
			CrudUtils.mapToObject(map, acrossEngineering,  "id");
		}else{
			CrudUtils.transMap2Bean(map, acrossEngineering);
		}
		return acrossEngineeringRepository.save(acrossEngineering);
	}
	@Override
	...
	private QueryResultObject querySingle(RequestCondition queryCondition) {
		...
		Specification<AcrossEngineering> specification = new Specification<AcrossEngineering>() {
			@Override
			public Predicate toPredicate(Root<AcrossEngineering> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		Page<AcrossEngineering> acrossEngineering = acrossEngineeringRepository.findAll(specification,request);
		List<AcrossEngineering> result = new ArrayList<AcrossEngineering>();
		long count = 0;
		if(null != qList && !qList.isEmpty()){
			result = acrossEngineering.getContent();
			count = acrossEngineering.getTotalElements();
		}
