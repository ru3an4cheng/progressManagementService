...
import com.progress.progress.services.IMovedEngineeringService;
import com.progress.progress.vo.MovedEngineeringVO;
...
@RestController
@Transactional
@RequestMapping("/movedEngineering")
public class MovedEngineeringController {
	/** 
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(MovedEngineeringController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
     ...
     */
	@Autowired
	private IMovedEngineeringService movedEngineeringService;
	/**
	 * @getById:根据id查询
	 * @param id
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = movedEngineeringService.getMovedEngineeringById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			movedEngineeringService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(movedEngineeringService.saveMovedEngineering(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = movedEngineeringService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, MovedEngineeringVO.class);
