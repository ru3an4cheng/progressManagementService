...
import com.progress.progress.services.IProjectSubitemService;
import com.progress.progress.vo.ProjectSubitemVO;
...
@RestController
@Transactional
@RequestMapping("/projectSubitem")
public class ProjectSubitemController {
	/** 
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(ProjectSubitemController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = projectSubitemService.getProjectSubitemById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			projectSubitemService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(projectSubitemService.saveProjectSubitem(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = projectSubitemService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, ProjectSubitemVO.class);
