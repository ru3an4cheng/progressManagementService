...
import com.progress.progress.services.IConstructionCompanyService;
import com.progress.progress.vo.ConstructionCompanyVO;
...
public class ConstructionCompanyController {
     ...
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(ConstructionCompanyController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	/** 
     * ConstructionCompany服务
     */
	@Autowired
	private IConstructionCompanyService constructionCompanyService;
	/**
	 * @getById:根据id查询
	 * @param id
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = constructionCompanyService.getConstructionCompanyById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			constructionCompanyService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(constructionCompanyService.saveConstructionCompany(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = constructionCompanyService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, ConstructionCompanyVO.class);
