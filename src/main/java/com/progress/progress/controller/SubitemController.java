...
import com.progress.progress.services.ISubitemService;
import com.progress.progress.vo.SubitemVO;
...
public class SubitemController {
     ...
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(SubitemController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = subitemService.getSubitemById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			subitemService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(subitemService.saveSubitem(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = subitemService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, SubitemVO.class);
