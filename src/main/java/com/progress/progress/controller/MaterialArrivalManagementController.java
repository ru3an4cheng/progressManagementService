...
import com.progress.progress.services.IMaterialArrivalManagementService;
import com.progress.progress.vo.MaterialArrivalManagementVO;
...
@RestController
@Transactional
@RequestMapping("/materialArrivalManagement")
public class MaterialArrivalManagementController {
	/** 
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(MaterialArrivalManagementController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	/** 
     * MaterialArrivalManagement服务
     */
	@Autowired
	private IMaterialArrivalManagementService materialArrivalManagementService;
	/**
	 * @getById:根据id查询
	 * @param id
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = materialArrivalManagementService.getMaterialArrivalManagementById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			materialArrivalManagementService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(materialArrivalManagementService.saveMaterialArrivalManagement(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = materialArrivalManagementService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, MaterialArrivalManagementVO.class);
