...
import com.progress.progress.services.INotificationService;
import com.progress.progress.vo.NotificationVO;
...
public class NotificationController {
     ...
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(NotificationController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = notificationService.getNotificationById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			notificationService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(notificationService.saveNotification(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = notificationService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, NotificationVO.class);
