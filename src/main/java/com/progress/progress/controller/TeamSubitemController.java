...
import com.progress.progress.services.ITeamSubitemService;
import com.progress.progress.vo.TeamSubitemVO;
...
@RestController
@Transactional
@RequestMapping("/teamSubitem")
public class TeamSubitemController {
	/** 
     * 日志
     */
	private final static Logger logger = (Logger) LoggerFactory.getLogger(TeamSubitemController.class);
	/**
	 * 方法绑定属性中不允许的参数
	 */
     ...
     */
	@Autowired
	private ITeamSubitemService teamSubitemService;
	/**
	 * @getById:根据id查询
	 * @param id
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = teamSubitemService.getTeamSubitemById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			teamSubitemService.remove(idObject);
			logger.info("删除成功");  
		...
		try {
			...
			if(items != null && !items.isEmpty()){
				for(Map<String,Object> map : items){
					result.setFormItems(teamSubitemService.saveTeamSubitem(map));
				}
			}
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		try {
			QueryResultObject queryResult = teamSubitemService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, TeamSubitemVO.class);
